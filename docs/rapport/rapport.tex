...
\title {Rapport Projet NachOS \\ M1 Informatique \\ Groupe G}
	...
	\subsection{Tests effectués et comportements}
		...
		\subsubsection{Multithreading}
			\begin{itemize}
				...
				\item Lancement d'une fonction: Testé avec les fonctions disponibles, elles se sont déroulées correcetement.
			\end{itemize}
		...
		\subsubsection{Mémoire virtuelle}
			\begin{itemize}
				...
				\item Stress de la mémoire
					\subitem Les fichiers matmult et sort s'éxécutent sans problèmes.
...
la position dans les dossier n'est pas sauvegarder entre deux éxécutions.Nous avons rajouté un OpenFile pour gérer le repertoire courant, celui ci pointe sur la racine lors du lancement de la machine, et est 		modifié via l'appel system cd.
à la racine, .. représente le dossier actuel.
