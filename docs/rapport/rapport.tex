	...
	\subsection{Tests effectués et comportements}
		\subsubsection{Console}
			\begin{itemize}
				...
				\item Utilisation de la console				validé : la console réécrit tout ce qui est tapé correctement
				\item Ecriture/lecture parallèle: Les chaînes de caractères sont écrites/lues d'un coup quelque soit l'ordonnancement, les lectures/écritures de caractères seuls peuvent être entrelacés selon l'ordonnancement.
		...
		\subsubsection{Multithreading}
			\begin{itemize}
				...
				\item Terminaison d'un thread:
					...
					\subitem Terminaison automatique: Tous les threads se terminent bien à la terminaison de leur processus même s'ils ne se sont pas explicitement terminés par UserThreadExit().
		...
		\subsubsection{Système de fichiers}
			\begin{itemize}
				\item Créer un dossier/fichier: Le dossier/fichier est créé et est accessible.
...
\section{Choix d'implémentation}
	...
	\subsection{Threads}
		...
		\subsubsection{Implémentation du join}
			Un vecteur contenant la liste des threads en attente et l'identifiant des threads qui les attends.
				...
				\end{itemize}
			~\par{Dès l'appel à \texttt{Join} pour le thread ayant pour identifiant \texttt{x} et si le thread x est en cours d'éxécution on ajoute la paire (thread courant, x) à la liste des threads en attente et le thread courant se met en pause. 
	...
	\subsection{Mémoire virtuelle}
		...
		\subsubsection{Multiprocessus}
			...
			\subsubsection{Shell}
			{Le shell implémenté permet de lancer un seul programme utilsateur à la fois.La valeur de retour de l'appel système \texttt{ForkExec} permet de déterminer si la commande est valide.}
			~\par{Lorsque le nouveau processus est lancé, le shell fait un appel à \texttt{Yield} tant que le shell n'est pas le seul processus actif.}
	...
	\subsection{Système de fichiers}
		...
		~\par{Concernant le déplacement dans l'arborescence de repertoire, nous avons ajouté un nouveau \texttt{OpenFile} vers le repertoire courant. Celui-ci est modifié uniquement par l'appel système \texttt{cd}.}
		~\par{Les chemins sont pris en compte par la fonction \texttt{MoveTo} qui navigue dans les repertoires jusqu'à celui demandé. on peut spécifié un chemin absolu en rajoutant "\texttt{/}" devant le chemin, sinon le chemin est relatif.}
		
		
	\subsection{Réseaux}
		\subsubsection{En-tête du message}
			{En plus des informations déjà présentes dans l'en-tête d'un paquet, le type du message (message de données (\texttt{MSG}) ou message d'acquittement (\texttt{ACK}), ainsi qu'un identifiant séquentiel unique parmi les messages envoyés de la machine en cours, sont ajoutés dans cet en-tête.}
...
\section{Organisation du travail}
		...
		~\par{Afin de valider une certaine implémentation, les tests unitaires devait tous réussir.}
		~\par{Des outils tels \emph{gdb} ainsi que \emph{valgrind} ont permis une détection beaucoup plus rapide des bugs et des fuites mémoire.}
